...
def backproject_pixel_to_ground_with_distortion(u, v, K, D, R, t, Z):
    ...
    d_world = -Z
    n_cam = R.T @ n_world
    d_cam = n_world @ t + d_world
    lam = -d_cam / (n_cam @ ray_cam)
    X_cam = lam * ray_cam
    X_world = R @ X_cam + t
    return X_world

def backproject_pixel_to_ground(u, v, K, R, t, Z):
    fx, fy = K[0,0], K[1,1]
    cx, cy = K[0,2], K[1,2]
    x_n = (u - cx) / fx
    y_n = (v - cy) / fy
...
def main():
    ...
    # I assume the even clicked points are the bases and the odd the respective tips
    real_world_points_base = np.empty((0, 3))
    real_world_points_tips = np.empty((0, 3))

    j = 1
    for i, (u, v) in enumerate(state['clicked_points']):
        if i % 2 == 0:
            z = 0.0 # base ground level
            if ACCOUNT_FOR_DISTORTION:
                X_world = backproject_pixel_to_ground_with_distortion(u, v, K=intrinsics, D=distortion, R=rotation, t=translation, Z=z)
            else:
                X_world = backproject_pixel_to_ground(u, v, K=intrinsics, R=rotation, t=translation, Z=z)
            X_world = X_world.reshape(-1)
            real_world_points_base = np.vstack([real_world_points_base, X_world])
            formatted = ", ".join(f"{x:.2f}" for x in X_world)
            print(f"{i+1:02d}. Pixel ({u}, {v}) -> World: Cone_base_{j} = ({formatted})")
            ...
            if ACCOUNT_FOR_DISTORTION:
                ...
                X_world = backproject_pixel_to_ground(u, v, K=intrinsics, R=rotation, t=translation, Z=z)
            X_world = X_world.reshape(-1)
            real_world_points_tips = np.vstack([real_world_points_tips, X_world])
            formatted = ", ".join(f"{x:.2f}" for x in X_world)
            print(f"{i+1:02d}. Pixel ({u}, {v}) -> World: Cone_tip_{j} = ({formatted})")
            j += 1

    X_base = real_world_points_base[:, 0]
    Y_base = real_world_points_base[:, 1]

    X_tip = real_world_points_tips[:, 0]
    Y_tip = real_world_points_tips[:, 1]
